...
var ElectronicWalletSession = function (opts) {
    if (!(this instanceof ElectronicWalletSession)) return new ElectronicWalletSession(opts);
    opts = opts || {};
    ...
    this.sessDefault = {
        ...
        servicePath: settings.get('service_bin', 'Zent-service'),
        configUpdated: false,
...
ElectronicWalletSession.prototype.get = function (key) {
    key = key || false;
    if (!key) {
...
ElectronicWalletSession.prototype.getDefault = function (key) {
    if (!key) {
...
ElectronicWalletSession.prototype.set = function (key, val) {
    if (!this.keys.includes(key)) {
        throw new Error(`Invalid session key: ${key}`);
    }
...
ElectronicWalletSession.prototype.reset = function (key) {
    if (key) {
...
ElectronicWalletSession.prototype.destroy = function () {
    return sessionStorage.removeItem(this.sessKey);
};

module.exports = ElectronicWalletSession;