...
const ElectronicWalletSession = require('./ws_session');
const ElectronicWalletApi = require('./ws_api');
...
const wsession = new ElectronicWalletSession(sessConfig);
...
var ElectronicWalletManager = function () {
    if (!(this instanceof ElectronicWalletManager)) {
        return new ElectronicWalletManager();
    }
...
ElectronicWalletManager.prototype.getUnusedPort = function () {
...
ElectronicWalletManager.prototype.init = function () {
    this._getSettings();
    ...
    this.serviceApi = new ElectronicWalletApi(cfg);
}).catch((err) => {
...
ElectronicWalletManager.prototype._getSettings = function () {
    let nodeAddress = settings.get('node_address').split(':');
    this.daemonHost = nodeAddress[0] || null;
    this.daemonPort = nodeAddress[1] || null;
    this.serviceBin = settings.get('service_bin');
...
ElectronicWalletManager.prototype._reinitSession = function () {
    this._wipeConfig();
    wsession.reset();
    this.notifyUpdate({
        type: 'sectionChanged',
...
ElectronicWalletManager.prototype._serviceBinExists = function () {
    wsutil.isFileExist(this.serviceBin);
};

// check 
ElectronicWalletManager.prototype.serviceStatus = function () {
    return (undefined !== this.serviceProcess && null !== this.serviceProcess);
};

ElectronicWalletManager.prototype.isRunning = function () {
    this.init();
...
ElectronicWalletManager.prototype._writeIniConfig = function (cfg) {
...
ElectronicWalletManager.prototype._writeConfig = function (cfg) {
    let configFile = wsession.get('walletConfig');
    if (!configFile) return '';
...
ElectronicWalletManager.prototype._wipeConfig = function () {
    try { fs.unlinkSync(wsession.get('walletConfig')); } catch (e) { }
};

ElectronicWalletManager.prototype.startService = function (walletFile, password, onError, onSuccess, onDelay) {
    this.init();
...
ElectronicWalletManager.prototype._argsToIni = function (args) {
    let configData = "";
...
ElectronicWalletManager.prototype._spawnService = function (walletFile, password, onError, onSuccess, onDelay) {
    this.init();
    ...
    this.serviceProcess.on('exit', (code, signal) => {
        serviceDown = true;
        log.debug(`zent service exit with code: ${code}, signal: ${signal}`);
    });
...
ElectronicWalletManager.prototype.stopService = function () {
...
ElectronicWalletManager.prototype.terminateService = function (force) {
...
ElectronicWalletManager.prototype.startSyncWorker = function () {
    this.init();
    let wsm = this;
...
ElectronicWalletManager.prototype.stopSyncWorker = function () {
...
ElectronicWalletManager.prototype.getNodeFee = function () {
    let wsm = this;
...
ElectronicWalletManager.prototype.genIntegratedAddress = function (paymentId, address) {
    let wsm = this;
...
ElectronicWalletManager.prototype.createWallet = function (walletFile, password) {
    this.init();
    let wsm = this;
    return new Promise((resolve, reject) => {
...
ElectronicWalletManager.prototype.importFromKeys = function (walletFile, password, viewKey, spendKey, scanHeight) {
    this.init();
    let wsm = this;
    return new Promise((resolve, reject) => {
...
ElectronicWalletManager.prototype.importFromSeed = function (walletFile, password, mnemonicSeed, scanHeight) {
    this.init();
    let wsm = this;
    return new Promise((resolve, reject) => {
        scanHeight = scanHeight || 0;
...
ElectronicWalletManager.prototype.getSecretKeys = function (address) {
    let wsm = this;
    return new Promise((resolve, reject) => {
...
ElectronicWalletManager.prototype.sendTransaction = function (params) {
    let wsm = this;
    return new Promise((resolve, reject) => {
...
ElectronicWalletManager.prototype.rescanWallet = function (scanHeight) {
    let wsm = this;
...
ElectronicWalletManager.prototype._fusionGetMinThreshold = function (threshold, minThreshold, maxFusionReadyCount, counter) {
    let wsm = this;
    return new Promise((resolve, reject) => {
        counter = counter || 0;
...
ElectronicWalletManager.prototype._fusionSendTx = function (threshold, counter) {
    let wsm = this;
...
ElectronicWalletManager.prototype.optimizeWallet = function () {
    let wsm = this;
...
ElectronicWalletManager.prototype.networkStateUpdate = function (state) {
    ...
    if (state === 0) {
        ...
        this.init();
        // looks like Zent-service always stalled after disconnected, just kill & relaunch it
...
ElectronicWalletManager.prototype.notifyUpdate = function (msg) {
    uiupdater.updateUiState(msg);
};

ElectronicWalletManager.prototype.resetState = function () {
    return this._reinitSession();
};

module.exports = ElectronicWalletManager;
