...
const ElectronicWalletAddressbook = require('./ws_addressbook');
...
let addressBook = new ElectronicWalletAddressbook(ADDRESS_BOOK_DEFAULT_PATH);
...
let jtfr = {
    tFind: [
        "Zent Cash Electronic Wallet",
        "Copyright  © 2020 Zent Cash Project TM",
        "Zent",
        "ZTC",
        "Zent-service",
        "CFG_MIN_FEE",
...
function showInitialPage() {
    ...
    let tsVersionInfo = document.getElementById('zentServiceVersion');
...
function handleSettings() {
    settingsButtonSave.addEventListener('click', function () {
        ...
        if (timeoutValue < 30 || timeoutValue > 240) {
            formMessageSet('settings', 'error', `Timeout value must be between 30 and 240`);
            return false;
        }
    ...
    function loadAddressBook(params) {
        ...
        if (params) {
            ...
            if (params.name === 'default') {
                addressBook = new ElectronicWalletAddressbook(ADDRESS_BOOK_DEFAULT_PATH);
            } else {
                addressBook = new ElectronicWalletAddressbook(params.path, params.name, params.pass);
            }
        }
...
ipcRenderer.on('cleanup', () => {
    ...
    let htmlText = 'Terminating ElectronicWallet...';
